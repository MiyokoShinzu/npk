<main class="col-md-12 col-lg-12 px-md-4 mx-auto" onload="initTables();">
  ...
  // Get plot_id from GET parameter
  $plot_id = isset($_GET['plot_id']) ? $_GET['plot_id'] : null;

  if (!$plot_id) {
      echo "No plot ID provided.";
      exit;
  }

  // Fetch plot info from database
  $sql = "SELECT * FROM plots WHERE id = '$plot_id' AND is_deleted = 0";
  ...
  <?php
      ...
      echo "Plot not found or has been deleted.";
  }
  ?>
</main>
...
<script>
  ...
  function printData() {
    ...
    autoTable(doc, { startY: 25, html: '#print-samples-table' });

    let nextY = doc.lastAutoTable.finalY || 10;
    doc.text(15, nextY + 10, "Nutrient Requirements for " + cropString);

    autoTable(doc, { startY: nextY + 15, html: '#print-nutrient-requirements' });

    nextY = doc.lastAutoTable.finalY || 10;
    doc.text(15, nextY + 10, "Fertilizer Recommendations for " + cropString);

    autoTable(doc, { startY: nextY + 15, html: '#print-fertilizer-recommendation' });
    ...
    const dateString = new Date(new Date().toLocaleString('en', { timeZone: 'Asia/Hong_Kong' }));
    let filename = "Plot ID " + plotID + " Soil Nutrient Data as of " + dateString;

    if (activeTabID) doc.save(filename + ".pdf");
    else alert("Select a crop type first.");
  }
